...
class PrestationsController < ApplicationController
  ...
  def create
    ...
    if not @prestation_foo.date.nil?
      ...
      unless params[:famille_id].blank?
       ...
       @enfants.count.times { |i|
          @enfant = @enfants[i]
          start_date = @prestation_foo.date.to_s(:en).to_date

          if params[:toutlemois] and (params[:lundi] or params[:mardi] or params[:mercredi] or params[:jeudi] or params[:vendredi])
               if params[:toutlemois]
                  ndays = days_in_month(start_date) - (start_date.day - 1)
               else
                  ndays = 5
               end
               date = start_date
	       ...
	       @prestation.garderieAM = @enfant.habitudeGarderieAM if @enfant.habitudeGarderieAM
	       @prestation.garderiePM = @enfant.habitudeGarderiePM if @enfant.habitudeGarderiePM 
               @prestation.date = @prestation_foo.date
               @prestation.totalP = 0.00
               @prestation.totalA = 0.00
               @prestation.save
            end
