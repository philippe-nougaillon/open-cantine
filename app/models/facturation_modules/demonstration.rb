...
module Facturation
  ...
  # Mairie "Démonstration" 	
  # Regroupe les calculs pour facturation des prestations 
  # Chaque établissement a son propre fichier 		

  def Facturation.best_tarif(kid)
    # Quel tarif apppliquer, automatique ou défini ?
    if kid.tarif_id # Tarif défini dans la fiche enfant
        tarif_id = Tarif.find(kid.tarif_id).type_id
    else
        tarif_id = 1 # on applique le tarif normal
        # sauf l'enfant n'est pas le premier de la famille, alors on applique le tarif famille
  ...
  def Facturation.tarif_majore(kid)
    # Quel tarif apppliquer, automatique ou majoré ?
    if kid.tarif_id
      tarif_id = Tarif.find(kid.tarif_id).type_id
    else
  ...
  def Facturation.calc_prestation(prestations_normales, prestation, tarif, jour)
	...
	# prestations_normales['Repas'] est le total des repas facturés	
	# prestations_normales['MntRepas'] est le montant total des repas facturés
	# prestations_normales['JoursRepas'] est la liste des jours de repas du mois 		

	# Etats des prestations : OUI = 1, ANNULE = 2, MAJORE = 3, ANNULE_RAISON_MALADIE = 4	
