...
module Facturation
  ...
  def Facturation.best_tarif(kid)
    ...
    if kid.tarif_id
        tarif_id = Tarif.find(kid.tarif_id).type_id
    else
        tarif_id = 1 #tarif normal
        # si l'enfant n'est pas le premier de la famille, tarif famille
        agemax = Enfant.maximum(:age, :conditions => ['famille_id = ?' , kid.famille_id])
        if kid.age != agemax
          tarif_id = 2 # tarif famille
        end
    end
    return tarif_id
  end
  ...
  def Facturation.tarif_majore(kid)
    if kid.tarif_id
      ...
      tarif_id = 3 #tarif MAJORÉ
      agemax = Enfant.maximum(:age, :conditions => ['famille_id = ?' , kid.famille_id])
      if kid.age != agemax
        tarif_id = 4 # tarif majoré famille
      end
    end
    return tarif_id
  end
  ...
  def Facturation.calc_prestation(prestations_normales, prestation, tarif, jour)
	 ...
	 prestations_normales['PrixRepas'] = tarif.RepasP
      ...
      #Garderie
      if (prestation.garderieAM == '5'or prestation.garderieAM == '1') #une heure
      ...
      if prestation.garderieAM == '6' #une heure trente
         prestations_normales['PrixGarderieAM'] = tarif.GarderieAMP 
         prestations_normales['MntGarderieAM'] += (tarif.GarderieAMP * 1.5)
         prestations_normales['GarderieAM'] += 1.5
         prestations_normales['JoursGarderieAM'] += "#{jour}, "
      end
      if prestation.garderieAM == '7' #deux heures
         prestations_normales['PrixGarderieAM'] = tarif.GarderieAMP 
         prestations_normales['MntGarderieAM'] = prestations_normales['MntGarderieAM'] + (tarif.GarderieAMP * 2)
         prestations_normales['GarderieAM'] += 2
         prestations_normales['JoursGarderieAM'] += "#{jour}, "
      end

      if (prestation.garderiePM == '5' or prestation.garderiePM == '1')
      ...
      if prestation.garderiePM == '6'
         prestations_normales['PrixGarderiePM'] = tarif.GarderiePMP 
         prestations_normales['MntGarderiePM'] += (tarif.GarderiePMP * 1.5)
         prestations_normales['GarderiePM'] += 1.5
         prestations_normales['JoursGarderiePM'] += "#{jour}, "
      end
      if prestation.garderiePM == '7'
         prestations_normales['PrixGarderiePM'] = tarif.GarderiePMP 
         prestations_normales['MntGarderiePM'] += (tarif.GarderiePMP * 2)
         prestations_normales['GarderiePM'] += 2
         prestations_normales['JoursGarderiePM'] += "#{jour}, "
      end

      #Centre
      if prestation.centreAM == '1' and prestation.centrePM == '1'
